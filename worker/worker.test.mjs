...
describe('buildButtonPollBlocks', () => {
  ...
  test('block count: header + prompt + options + marker (no description)', () => {
    ...
    // 1 header + 1 prompt + 3 options + 1 marker = 6
  ...
  test('each option block has type section', () => {
    const blocks = buildButtonPollBlocks(poll, {}, 'summer-sports');
    for (let i = 0; i < poll.options.length; i++) {
      assert.equal(blocks[i + 2].type, 'section');
    }
  });

  test('each option accessory has action_id poll_vote', () => {
    const blocks = buildButtonPollBlocks(poll, {}, 'summer-sports');
    for (let i = 0; i < poll.options.length; i++) {
      const { accessory } = blocks[i + 2];
      assert.ok(accessory, `block ${i + 2} missing accessory`);
      assert.equal(accessory.action_id, 'poll_vote');
    }
  });
  ...
  test('button values encode slug and option index', () => {
    const blocks = buildButtonPollBlocks(poll, {}, 'summer-sports');
    for (let i = 0; i < poll.options.length; i++) {
      assert.equal(blocks[i + 2].accessory.value, `summer-sports:${i}`);
    }
  });
